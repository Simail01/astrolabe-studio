  ...
  const menus: MenuDef[] = [
    {
      ...
      items: [
        ...
        { divider: true, label: '' },
        { label: '导出小说 (TXT)', action: async () => { close(); const ws = useWorkspaceStore.getState(); const pp = ws.getProjectPath(); if (!pp) return alert('请先选择作品'); await bridge.exportNovel(pp, 'txt'); } },
        { label: '导出小说 (EPUB)', action: async () => { close(); const ws = useWorkspaceStore.getState(); const pp = ws.getProjectPath(); if (!pp) return alert('请先选择作品'); await bridge.exportNovel(pp, 'epub'); } },
        { divider: true, label: '' },
