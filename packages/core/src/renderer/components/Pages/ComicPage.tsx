...
export const ComicPage: React.FC = () => {
  ...
  const assignShot = (shot: Shot) => {
    ...
    newPanels[selectedPanel] = { shotId: shot.id, shotTitle: `镜头${shot.order || selectedPanel + 1}`, imageUrl: (shot as any).notes || '' };
    setPanels(newPanels);
  };
  ...
  const generatePanel = async (idx: number) => {
    ...
    try {
      ...
      const prompt = `漫画风格，${shot.framing || 'medium'}，${shot.angle || 'eye-level'}。场景：${shot.scene || ''}。氛围：${shot.mood || ''}`;
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 16, overflow: 'auto' }}>
        ...
        {genMsg && !generating && (
          <div style={{ padding: '6px 12px', marginBottom: 8, backgroundColor: 'var(--accent-dim)', color: 'var(--accent)', fontSize: 12, borderRadius: 4 }}>{genMsg}</div>
        )}
      ...
      <div style={{ width: 300, backgroundColor: 'var(--bg-panel)', borderLeft: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 8 }}>
          {!shots.length ? (
            <div style={{ padding: 12, fontSize: 12, color: 'var(--text-muted)' }}>暂无分镜数据，请先在视觉化模式中生成分镜</div>
          ) : (
            shots.map((shot, i) => (
              <div key={shot.id || i} onClick={() => assignShot(shot)}
                ...
                {panels.some(p => p.shotId === shot.id) && (
                  <div style={{ fontSize: 10, color: 'var(--color-success)', marginTop: 2 }}>✓ 已分配</div>
                )}
              </div>
            ))
          )}
